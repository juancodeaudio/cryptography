...
import { PathLike, mkdir, mkdirSync, writeFileSync } from 'fs';
...
const keygen = (
  ...
  switch (type) {
    case 'rsa': {
      const options: crypto.RSAKeyPairOptions<crypto.KeyFormat, crypto.KeyFormat> = {
        modulusLength,
        publicKeyEncoding: { type: "spki", format },
        privateKeyEncoding: { type: "pkcs8", format, cipher: `aes-${size}-cbc`, passphrase }
      }
